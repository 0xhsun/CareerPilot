...
**Area filter (`areas`):** `AREA_OPTIONS` in `config.py` is the single source of truth — the frontend fetches it from `GET /api/jobs/options` and only falls back to its own hardcoded copy (`FALLBACK_OPTIONS` in `SearchPage` / `AlertsPage` / `SmartMatchPage`) when the API is unreachable. Values are 104 area codes, ordered as 104 orders them; the north is covered in full (台北、新北、宜蘭、基隆、桃園、新竹縣市、苗栗). Each scraper maps the code to its own site's value, so **adding an area means updating three maps**: `AREA_OPTIONS`, `_AREA_TO_YOURATOR_CODE`, `_AREA_TO_CAKE_CITY` (plus the three frontend fallbacks). Both scraper maps are list-valued because one 104 code can span several site values — `6001006000` is 新竹縣市 on 104 but splits into `HSZ`+`HSQ` on Yourator and 新竹市+新竹縣 on Cake. Note Yourator is startup-heavy and genuinely has **no** listings in 基隆/宜蘭/苗栗 (verified live 2026-09-17: `jobs: []`, valid code, empty result) — those areas return results from 104 only.
